...
public class GeneralPurposeFog extends PowerDatacenter {
    protected Queue<Tuple> northTupleQueue;
    protected Queue<Pair<Tuple, Integer>> southTupleQueue;

    protected List<String> activeSDFCApplications;

    protected Map<String, SDFCApplication> applicationMap;
    ...
    protected List<Pair<Integer, Double>> associatedSDFCActuatorIds;
    ...
    public GeneralPurposeFog(String name, FogDeviceCharacteristics characteristics, VmAllocationPolicy vmAllocationPolicy,
        ...
        setAssociatedSDFCActuatorIds(new ArrayList<Pair<Integer, Double>>());
        for (Host host : getCharacteristics().getHostList()) {
            host.setDatacenter(this);
        }
        setActiveSDFCApplications(new ArrayList<String>());
        ...
        applicationMap = new HashMap<String, SDFCApplication>();
    ...
    public GeneralPurposeFog(String name, long mips, int ram, double uplinkBandwidth, double downlinkBandwidth,
...
//     * <b>NOTE:</b> You do not need to override {@link #()} method, if you use
//     * this method.
//     *
//     * @pre $none
//     * @post $none
//     */
    ...
    protected void processOtherEvent(SimEvent ev) {
        switch (ev.getTag()) {
            ...
            case FogEvents.ACTIVE_APP_UPDATE:
                updateActiveSDFCApplications(ev);
                break;
            case FogEvents.ACTUATOR_JOINED:
                processSDFCActuatorJoined(ev);
                break;
    ...
    protected void processSDFCActuatorJoined(SimEvent ev) {
        int actuatorId = ev.getSource();
        double delay = (double) ev.getData();
        getAssociatedSDFCActuatorIds().add(new Pair<Integer, Double>(actuatorId, delay));
    }

    protected void updateActiveSDFCApplications(SimEvent ev) {
        SDFCApplication app = (SDFCApplication) ev.getData();
        getActiveSDFCApplications().add(app.getAppId());
    }
            ...
            for (Vm vm : host.getVmList()) {
                while (vm.getCloudletScheduler().isFinishedCloudlets()) {
                    ...
                    if (cl != null) {
                        ...
                        MainApplication.masterFog.interModuleCOmmunicationCost += tuple.getProcessingCost();
                        SDFCApplication application = getSDFCApplicationMap().get(tuple.getAppId());
                        ...
                        for (Tuple resTuple : resultantTuples) {
                            ...
                            //System.out.println("Sending to self "+resTuple.getSrcModuleName()+" "+resTuple.getDestModuleName());
                            sendToSelf(resTuple);
                        }
    ...
    protected void updateTimingsOnSending(Tuple resTuple) {
        ...
        for (AppLoop loop : getSDFCApplicationMap().get(resTuple.getAppId()).getLoops()) {
    ...
    protected int getChildIdForTuple(Tuple tuple) {
        if (tuple.getDirection() == Tuple.ACTUATOR) {
            int gatewayId = ((SDFCActuator) CloudSim.getEntity(tuple.getActuatorId())).getGatewayDeviceId();
...
//        System.out.println("-------------------------------\n");
    ...
    protected void sendTupleToSDFCActuator(Tuple tuple) {
        /*
         * for(Pair<Integer, Double> actuatorAssociation :
         * getAssociatedSDFCActuatorIds()){ int actuatorId =
         * actuatorAssociation.getFirst(); double delay =
         * actuatorAssociation.getSecond(); if(actuatorId == tuple.getSDFCActuatorId()){
         ...
         * childId);
         */
        for (Pair<Integer, Double> actuatorAssociation : getAssociatedSDFCActuatorIds()) {
            int actuatorId = actuatorAssociation.getFirst();
            double delay = actuatorAssociation.getSecond();
            String actuatorType = ((SDFCActuator) CloudSim.getEntity(actuatorId)).getSDFCActuatorType();
    ...
    protected void processTupleArrival(SimEvent ev) {
        ...
        if (getName().equals(Constant.CLOUD)) {
            updateCloudTraffic();
        }
         ...
         */
        Logger.debug(getName(),tuple.getDirection()+" "+
                ...
                +" "+ ev.getSource()+" "+ev.getDestination()
...
//            tuple.setDestModuleName(Constant.ACTUATOR);
//        }

        if (tuple.getDirection() == Tuple.ACTUATOR) {
            sendTupleToSDFCActuator(tuple);
            return;
        }
        ...
        if (getName().equals(Constant.CLOUD) && tuple.getDestModuleName() == null) {
    ...
    protected void updateTimingsOnReceipt(Tuple tuple) {
        SDFCApplication app = getSDFCApplicationMap().get(tuple.getAppId());
    ...
    private void initializePeriodicTuples(AppModule module) {
        String appId = module.getAppId();
        SDFCApplication app = getSDFCApplicationMap().get(appId);
    ...
    public List<String> getActiveSDFCApplications() {
        return activeSDFCApplications;
    }

    public void setActiveSDFCApplications(List<String> activeSDFCApplications) {
        this.activeSDFCApplications = activeSDFCApplications;
    }
    ...
    public Map<String, SDFCApplication> getSDFCApplicationMap() {
        return applicationMap;
    }

    public void setSDFCApplicationMap(Map<String, SDFCApplication> applicationMap) {
    ...
    public List<Pair<Integer, Double>> getAssociatedSDFCActuatorIds() {
        return associatedSDFCActuatorIds;
    }

    public void setAssociatedSDFCActuatorIds(List<Pair<Integer, Double>> associatedSDFCActuatorIds) {
        this.associatedSDFCActuatorIds = associatedSDFCActuatorIds;
    }
