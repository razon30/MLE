...
public class SDFCSensor extends SimEntity {
    ...
    public SDFCSensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation,
     ...
     */
    public SDFCSensor(String name, String tupleType, int userId, String appId, Distribution transmitDistribution) {
        super(name);
        this.setAppId(appId);
        this.setTransmitDistribution(transmitDistribution);
        setTupleType(tupleType);
    ...
    public SDFCSensor(String name, String tupleType, String appId, Distribution transmitDistribution) {
        super(name);
        this.setAppId(appId);
        this.setTransmitDistribution(transmitDistribution);
        setTupleType(tupleType);
    ...
    public void transmit() {
        AppEdge _edge = null;
        int randomModuleType = Utils.getValue(0, MainApplication.numberOfMS - 1);
        for (AppEdge edge : getApp().getEdges()) {
            if (edge.getSource().equals(getTupleType()) ){//&& (edge.getDestination().equals(Constant.CITIZEN_MODULE + randomModuleType))){
                _edge = edge;
                break;
            }
        }
...
//        System.out.println(gatewayDeviceId);
        ...
        // if (_edge.getDestination().equals("CitizenModule5")) return;
...
//        System.out.println("===========================");
        ...
        if (!MainApplication.masterFog.getPlacedMSIDbyCFID().containsKey(gatewayDeviceId)) {
            gatewayDeviceId = MainApplication.masterFog.provideGateWayDeviceId(_edge.getDestination());
        }
    ...
    private int updateTimings(String src, String dest) {
        SDFCApplication application = getApp();
