...
public class MasterComputingUnit extends PowerDatacenter {
	protected Queue<Tuple> northTupleQueue;
	protected Queue<Pair<Tuple, Integer>> southTupleQueue;

	protected List<String> activeSocialMediaApplications;

	protected Map<String, SocialMediaApplication> applicationMap;
	...
	public MasterComputingUnit(String name, FogDeviceCharacteristics characteristics,
		...
		setActiveSocialMediaApplications(new ArrayList<String>());
		...
		applicationMap = new HashMap<String, SocialMediaApplication>();
	...
	public MasterComputingUnit(String name, long mips, int ram, double uplinkBandwidth, double downlinkBandwidth,
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		case FogEvents.ACTIVE_APP_UPDATE:
			updateActiveSocialMediaApplications(ev);
			break;
	...
	private Microservice getModuleByName(String moduleName) {
		Microservice module = null;
		for (Vm vm : getHost().getVmList()) {
			if (((Microservice) vm).getName().equals(moduleName)) {
				module = (Microservice) vm;
				break;
			}
		}
		return module;
	...
	private void sendPeriodicTuple(SimEvent ev) {
		...
		Microservice module = getModuleByName(srcModule);
	...
	protected void updateActiveSocialMediaApplications(SimEvent ev) {
		SocialMediaApplication app = (SocialMediaApplication) ev.getData();
		getActiveSocialMediaApplications().add(app.getAppId());
	}

	public String getOperatorName(int vmId) {
		for (Vm vm : this.getHost().getVmList()) {
			if (vm.getId() == vmId)
				return ((Microservice) vm).getName();
		}
		return null;
	}
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						...
						SocialMediaApplication application = getSocialMediaApplicationMap().get(tuple.getAppId());
						...
						for (Tuple resTuple : resultantTuples) {
							...
							resTuple.getModuleCopyMap().put(((Microservice) vm).getName(), vm.getId());
	...
	protected void updateTimingsOnSending(Tuple resTuple) {
		...
		for (AppLoop loop : getSocialMediaApplicationMap().get(resTuple.getAppId()).getLoops()) {
			if (loop.hasEdge(srcModule, destModule) && loop.isStartModule(srcModule)) {
				...
				Logger.debug(getName(),
						"\tSENDING\t" + resTuple.getActualTupleId() + "\tSrc:" + srcModule + "\tDest:" + destModule);
	...
	protected int getChildIdForTuple(Tuple tuple) {
		if (tuple.getDirection() == Tuple.ACTUATOR) {
			int gatewayId = ((ActuatorEndDevice) CloudSim.getEntity(tuple.getActuatorId())).getGatewayDeviceId();
	...
	protected void updateAllocatedMips(String incomingOperator) {
		...
		for (final Vm vm : getHost().getVmList()) {
			if (vm.getCloudletScheduler().runningCloudlets() > 0
					|| ((Microservice) vm).getName().equals(incomingOperator)) {
	...
	private void updateEnergyConsumption() {
		...
		for (final Vm vm : getHost().getVmList()) {
			Microservice operator = (Microservice) vm;
	...
	protected void sendTupleToMyActuator(Tuple tuple) {
		 ...
		 * getAssociatedMyActuatorIds()){ int actuatorId =
		 * actuatorAssociation.getFirst(); double delay =
		 * actuatorAssociation.getSecond(); if(actuatorId == tuple.getMyActuatorId()){
		...
		Logger.debug("Tuple ID: " + tuple.getActualTupleId(), "Sendtiong to " + tuple.getDestModuleName());
		...
		for (Pair<Integer, Double> actuatorAssociation : getAssociatedMyActuatorIds()) {
			...
			String actuatorType = ((ActuatorEndDevice) CloudSim.getEntity(actuatorId)).getMyActuatorType();
	...
	protected void processTupleArrival(SimEvent ev) {
		...
		Utils.print("Fog name before updateCloudTraffic(): " + getName());

		// if (getName().equals(tuple.)) {
		updateCloudTraffic();
		// }
...
//		if (getName().equalsIgnoreCase(Constants.MASTER_FOG)) {
//			sendTupleToexecutingModule(tuple);
//			return;
//		}

		if (getHost().getVmList().size() > 0) {
			final Microservice operator = (Microservice) getHost().getVmList().get(0);
			if (CloudSim.clock() > 0) {
...
//		if (getName().equalsIgnoreCase(MasterFog.getTupleToExecutingModuleNameMap().get(tuple.getCloudletId()))
...
//		}
		...
		if (appToModulesMap.containsKey(tuple.getAppId())) {
			if (appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName())) {
				int vmId = -1;
				for (Vm vm : getHost().getVmList()) {
					if (((Microservice) vm).getName().equals(tuple.getDestModuleName())) {
						vmId = vm.getId();
					}
				}
				...
				tuple.setVmId(vmId);
				Logger.error(getName(), "Executing tuple for operator " + getName());
	...
	private void sendTupleToexecutingModule(Tuple tuple) {
		// TODO Auto-generated method stub

		String destName = MasterFog.getTupleToExecutingModuleNameMap().get(tuple.getCloudletId());

		tuple.setDestModuleName(destName);
		tuple.setSrcModuleName(Constants.MAIN_MODULE);

		// sendUp(tuple);

		send(destName, 0, FogEvents.TUPLE_FINISHED, tuple);

	}

	protected void updateTimingsOnReceipt(Tuple tuple) {
		SocialMediaApplication app = getSocialMediaApplicationMap().get(tuple.getAppId());
	...
	protected void executeTuple(SimEvent ev, String moduleName) {
		...
		Microservice module = getModuleByName(moduleName);
		...
		if (moduleName.equalsIgnoreCase(Constants.MAIN_MODULE)) {
			sendTupleToexecutingModule(tuple);
		}

		/*
		 * for(Vm vm : getHost().getVmList()){ Logger.error(getName(),
		 * "MIPS allocated to "+((Microservice)vm).getName()+" = "+getHost().
		 * getTotalAllocatedMipsForVm(vm)); }
		 */
	}

	protected void processModuleArrival(SimEvent ev) {
		Microservice module = (Microservice) ev.getData();
	...
	private void initializePeriodicTuples(Microservice module) {
		String appId = module.getAppId();
		SocialMediaApplication app = getSocialMediaApplicationMap().get(appId);
	...
	protected void sendToSelf(Tuple tuple) {
		Logger.debug("Tuple ID: " + tuple.getActualTupleId(), "Sending to " + tuple.getDestModuleName());
	...
	public List<String> getActiveSocialMediaApplications() {
		return activeSocialMediaApplications;
	}

	public void setActiveSocialMediaApplications(List<String> activeSocialMediaApplications) {
		this.activeSocialMediaApplications = activeSocialMediaApplications;
	}
	...
	public Map<String, SocialMediaApplication> getSocialMediaApplicationMap() {
		return applicationMap;
	}

	public void setSocialMediaApplicationMap(Map<String, SocialMediaApplication> applicationMap) {
	...
	public FogDeviceCharacteristics getFogCharacteristics() {
		return (FogDeviceCharacteristics) getCharacteristics();
	}
