...
public class SDFCController extends SimEntity {
	...
	private Map<String, SDFCModulePlacement> appModulePlacementPolicy;

	public SDFCController(String name, List<SDFCFogDevice> fogDevices, List<SDFCSensor> sensors,
			List<SDFCActuator> actuators) {
		super(name);
		this.applications = new HashMap<String, SDFCApplication>();
		setAppLaunchDelays(new HashMap<String, Integer>());
		setAppModulePlacementPolicy(new HashMap<String, SDFCModulePlacement>());
		for (SDFCFogDevice fogDevice : fogDevices) {
			fogDevice.setControllerId(getId());
		}
		setSDFCFogDevices(fogDevices);
		setSDFCActuators(actuators);
		setSDFCSensors(sensors);
		connectWithLatencies();
	}

	private SDFCFogDevice getSDFCFogDeviceById(int id) {
		for (SDFCFogDevice fogDevice : getSDFCFogDevices()) {
	...
	private void connectWithLatencies() {
		for (SDFCFogDevice fogDevice : getSDFCFogDevices()) {
			SDFCFogDevice parent = getSDFCFogDeviceById(fogDevice.getParentId());
			if (parent == null)
				continue;
	...
	private void printResourceDetails() {
		...
		for (SDFCFogDevice mainCitizenFog: SimulateSDFC.CFbyIDHealthStatusTable.values()){
...
//		System.out.println("-------------------------------------------------------------");
	...
	private SDFCFogDevice getCloud() {
		for (SDFCFogDevice dev : getSDFCFogDevices())
			if (dev.getName().equals(Constant.MASTER_FOG + "0"))
				return dev;
		return null;
	}
	...
	public void submitApplication(SDFCApplication application, int delay, SDFCModulePlacement modulePlacement) {
		...
		for (AppEdge edge : application.getEdges()) {
			if (edge.getEdgeType() == AppEdge.ACTUATOR) {
				...
				for (SDFCActuator actuator : getSDFCActuators()) {
	...
	public void submitApplication(SDFCApplication application, SDFCModulePlacement modulePlacement) {
	...
	private void processAppSubmit(SDFCApplication application) {
		...
		SDFCModulePlacement modulePlacement = getAppModulePlacementPolicy().get(application.getAppId());
		for (SDFCFogDevice fogDevice : fogDevices) {
		...
		for (Integer deviceId : deviceToModuleMap.keySet()) {
			for (AppModule module : deviceToModuleMap.get(deviceId)) {
				...
				System.out.println(CloudSim.clock() + " Trying to Launch " + module.getName() + " in "
						+ getSDFCFogDeviceById(deviceId).getName());
	...
	public Map<String, SDFCModulePlacement> getAppModulePlacementPolicy() {
		return appModulePlacementPolicy;
	}

	public void setAppModulePlacementPolicy(Map<String, SDFCModulePlacement> appModulePlacementPolicy) {
