...
public class SDFCFogDevice extends PowerDatacenter {
	...
	public SDFCFogDevice(String name, FogDeviceCharacteristics characteristics, VmAllocationPolicy vmAllocationPolicy,
			List<Storage> storageList, double schedulingInterval, double uplinkBandwidth, double downlinkBandwidth,
			double uplinkLatency, double ratePerMips, float executionTime) throws Exception {
	...
	public SDFCFogDevice(String name, long mips, int ram, double uplinkBandwidth, double downlinkBandwidth,
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		case FogEvents.TUPLE_ARRIVAL:
			...
			break;
		case FogEvents.EMIT_FROM_CF:
			processFromCF(ev);
			break;
		case FogEvents.EMIT_FROM_MF:
			processFromMF(ev);
			break;
		case FogEvents.RESPONSE_FROM_CF:
			responseFromCF(ev);
			break;
		case FogEvents.RESPONSE_FROM_MF:
			responseFromMF(ev);
			break;
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						...
						SDFCApplication application = getApplicationMap().get(tuple.getAppId());
	...
	public int getChildIdWithRouteTo(int targetDeviceId) {
		for (Integer childId : getChildrenIds()) {
			if (targetDeviceId == childId)
				return childId;
			if (((SDFCFogDevice) CloudSim.getEntity(childId)).getChildIdWithRouteTo(targetDeviceId) != -1)
				return childId;
		}
		return -1;
	}
	...
	protected void processTupleArrival(SimEvent ev) {
		...
		if (!getName().equalsIgnoreCase(Constant.MASTER_FOG + "0")) {

			if (isExecutable(tuple)) {
//
...
//					System.out.println("\nSending, My Name is: " + getName());
//					send(Constant.MASTER_FOG + "0", 0, FogEvents.EMIT_FROM_CF, tuple);
//					return;
//				}

			} else {
				MNH.addRequester(getName(), tuple.getCloudletId() + "");
				tuple.setDestModuleName(Constant.MASTER_MODULE);
				// System.out.println("\nSending, My Name is: " + getName());
				send(Constant.MASTER_FOG + "0", 0, FogEvents.EMIT_FROM_CF, tuple);
				return;
...
//		if (getName().equalsIgnoreCase(Constant.MASTER_FOG + "0")) {
//			tuple.setDestModuleName(Constant.CITIZEN_MODULE);
//			System.out.println("\nSending, My Name is: " + getName());
//			send(Constant.CITIZEN_FOG + "1", 0, FogEvents.EMIT_FROM_CF, tuple);
//			return;
//		}
//
		if (tuple.getDirection() == Tuple.ACTUATOR) {
...
//		}
	...
	private boolean isExecutable(Tuple tuple) {
		int requiredPes = tuple.getNumberOfPes();
		long requiredMIPS = tuple.getCloudletLength();

		if (getHost().getNumberOfFreePes() >= requiredPes && getHost().getAvailableMips() >= requiredMIPS) {
			return true;
		}

//		System.out.println("requiredPes: " + requiredPes + "   available: " + getHost().getNumberOfFreePes());
//		System.out.println("requiredMIPS: " + requiredPes + "   available: " + getHost().getAvailableMips());

		return false;
	}

	private void processFromCF(SimEvent ev) {
		Tuple tuple = (Tuple) ev.getData();

		// System.out.println("Came here, My Name is: " + getName());
//
		// if (getName().equalsIgnoreCase(Constant.MASTER_FOG + "0")) {

		SDFCFogDevice destDevice = MNH.sortedDevices.get(0);

		tuple.setDestModuleName(Constant.CITIZEN_MODULE);
		// System.out.println("\nSending, My Name is: " + getName());
		send(destDevice.getName(), 0, FogEvents.EMIT_FROM_MF, tuple);
		// return;
		// }

	}

	private void processFromMF(SimEvent ev) {
		Tuple tuple = (Tuple) ev.getData();

		// System.out.println("Came here, My Name is: " + getName());

		executeTupleWithName(ev, tuple.getDestModuleName());

		tuple.setDestModuleName(Constant.MASTER_MODULE);
		send(Constant.MASTER_FOG + "0", 0, FogEvents.RESPONSE_FROM_CF, tuple);

	}

	private void responseFromCF(SimEvent ev) {
		Tuple tuple = (Tuple) ev.getData();
		// System.out.println("Came here, My Name is: " + getName());
		...
		String receiver = MNH.getRequester(tuple.getCloudletId() + "");

		tuple.setDestModuleName(Constant.CITIZEN_MODULE);
		// System.out.println("\nSending, My Name is: " + getName());
		send(receiver, 0, FogEvents.RESPONSE_FROM_MF, tuple);
		// return;
		// }

	}

	private void responseFromMF(SimEvent ev) {
		Tuple tuple = (Tuple) ev.getData();
		...
		updateAllocatedMips(MNH.updatedDeviceToModule.get(getName()).get(0));

//		updateTimingsOnReceipt(tuple);
//
//		TimeKeeper.getInstance().tupleStartedExecution(tuple);
//		updateAllocatedMips(Constant.CITIZEN_MODULE);
//		processCloudletSubmit(ev, false);
//		updateAllocatedMips(Constant.CITIZEN_MODULE);
	...
	protected void updateTimingsOnReceipt(Tuple tuple) {
		SDFCApplication app = getApplicationMap().get(tuple.getAppId());
	...
	protected void executeTupleWithName(SimEvent ev, String moduleName) {
		Logger.debug(getName(), "Executing tuple on module " + moduleName + " Installed in CF: " + getName());
		Tuple tuple = (Tuple) ev.getData();
	...
	private void initializePeriodicTuples(AppModule module) {
		String appId = module.getAppId();
		SDFCApplication app = getApplicationMap().get(appId);
	...
	public Map<String, SDFCApplication> getApplicationMap() {
		return applicationMap;
	}

	public void setApplicationMap(Map<String, SDFCApplication> applicationMap) {
