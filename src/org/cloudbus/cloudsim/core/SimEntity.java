...
public abstract class SimEntity implements Cloneable {
	...
	//public int numberOfTuplePerEvent = 1;

	public void run() {
		SimEvent ev = evbuf != null ? evbuf : getNextEvent();

//		if (ev != null && ev.getTag() == FogEvents.EMIT_TUPLE) {
//			for (int i = 0; i < numberOfTuplePerEvent; i++) {
//				processEvent(ev);
//			}
//		}

		while (ev != null) {
//			if (ev.getTag() == FogEvents.EMIT_TUPLE || ev.getTag() == FogEvents.RE_EMIT_TUPLE) {
//				MainApplication.tupleEventCounter += 1;
//				processEvent(ev);
//				//return;
//			}else {
				processEvent(ev);
