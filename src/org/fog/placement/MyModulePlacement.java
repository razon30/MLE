...
public class MyModulePlacement extends MyPlacement {
    ...
    protected void mapModules() {
        ...
        for (String deviceName : getModuleMapping().getModuleMapping().keySet()) {
            ...
            for (String moduleName : getModuleMapping().getModuleMapping().get(deviceName)) {
				...
				if (moduleName.contains("storageModule")){
				...
				}else if (isPlacementPossible(deviceId, appModule)) {

					TestApplication.placedNumberOfModule++;
                ...
                for (int key : keys) {
                    ...
                    if (currentMips + baseMipsOfPlacingModule + additionalMips < device.getMips()) {
                        ...
                        if (placedModules == null){
							//System.out.println("placedmodule");

							List<AppModule> appModules = new ArrayList<>();
							appModules.add(appModule);
							getDeviceToModuleMap().put(deviceParent, appModules);
						}else {
							placedModules.add(appModule);
    ...
    public boolean isPlacementPossible(int id, AppModule container) {
...
//        System.out.println("==================================================================");
        ...
        if (citizenFog == null) return false;

        if ((citizenFog.getMips() >= container.getMips()) &&
